...
- Successivo univoco proposto, in attesa di approvazione: `GUI-U-R03-R02 - Cartelle operative configurabili per casella`. Risultato: la GUI sostituisce `Virgilio_Inbox`, `Virgilio_Done` e `Virgilio_Errori` impliciti con valori avanzati comprensibili, persistenti e distinti per casella; il caso reale deve accettare `da-traghettare` senza cartella madre. Esclusioni: rete/credenziali reali nei test, modifica Apps Script, redesign e nuova build prima della chiusura.
