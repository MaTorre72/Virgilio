...
| `virgilio_inbox.gs` | `caronteRegistraVirgilioInbox` | Intake metadata-only idempotente della inbox | payload con `manifest` e metadati Drive verificati | create/update di una sola riga `Virgilio_Inbox` | `SpreadsheetApp`, `caronte_bridge.gs`, `drive_staging_verify.gs` | Si | usa `fingerprint` come chiave primaria e rifiuta intake senza file/manifest davvero visibili su Drive |
