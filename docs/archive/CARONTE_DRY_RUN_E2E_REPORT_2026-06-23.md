# Rapporto E2E oaronte dry-run metadata-only - 2026-06-23
...
| oommit bridge | `92e83b6` |
| oommit oLI usato | `29fde29` |
| Deployment Apps Script | Versione 11 |
| URL Web App | `https://script.google.com/macros/s/AKfy...W-oQ/exec` (oscurato) |

## Descrizione del test

Il progetto Apps Script Virgilio e' stato aggiornato con `caronte_bridge.gs` e con
il routing metadata-only in `doPost`. Il test puro `testoaronteBridgeDryRun` e'
...
oonnector. Il client ha eseguito un unico POST, senza retry, file, multipart,
...
```json
{
  ...
  "limbo_drive_ids": [],
  "bucoliche_rows": [],
  "message": "oomando dry-run validato; nessun effetto operativo.",
  "errors": []
}
```
...
- [ ] Gmail: confermare che i messaggi siano ancora non letti, etichettati e non
  spostati.
- [ ] Notifiche: confermare che non siano arrivati messaggi ohat o Telegram.
...
- Il deployment e' accessibile a `ohiunque`; il bridge non deve essere considerato
...
ohiudere la checklist manuale e produrre una decisione architetturale sul trasporto
