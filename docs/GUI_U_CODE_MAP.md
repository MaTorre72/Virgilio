...
Questa mappa classifica il codice della GUI tecnica esistente rispetto alla
nuova applicazione `virgilio_connector.user_app`. La classificazione non
autorizza import diretti dai moduli `gui`, `gui_*` o `maintenance_gui`: le regole
di dipendenza di `docs/GUI_U_ARCHITETTURA.md` restano vincolanti.
...
- `TECNICO`: appartiene esclusivamente a `Caronte Manutenzione`;
...
| --- | --- | --- | --- | --- |
| `maintenance_gui.py` | entry point stabile che riesporta la GUI tecnica | `test_gui.py` | `TECNICO` | resta entry point di `Caronte Manutenzione` |
| `gui.py` | finestra Tk tecnica, nove tab, registro azioni, costruzione argomenti ed esecuzione comandi | `test_gui.py` | `TECNICO` | resta implementazione della sola manutenzione; nessun widget, tab o registro passa a `user_app` |
...
CLI, `user_app` e `maintenance_gui` sono consumer dei servizi condivisi. Solo
flusso, navigazione, widget e testi finali hanno destinazione esclusivamente
utente; registro delle nove tab, argomenti, output e codici di ritorno restano
...
- Completamento E3: attivita` e problemi; preferenze essenziali; Bucoliche e
  avvio Windows guidati; manutenzione protetta; build e installer autonomi
  (`GUI-U-E3-T01..T06`).
