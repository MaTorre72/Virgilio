...
- Modalita`: programma `CONS` seriale, un task e massimo un commit per run;
  fetch/pull solo fast-forward e push non forzato.
- Baseline preservata: commit `7e18277`, `PASS` umano 2026-07-28, Apps Script
  deployment `40`.
- Release: versione `1.1.0`; installer ufficiale prodotto e verificato; tag
...
- Ultimo task: `CONS-H03` `DONE`; programma `CONS` completato e pull request
