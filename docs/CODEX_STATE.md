...
- Task corrente di codice: `GUI-U-E3-T09 - Accesso alle caselle Google`
- Task corrente consigliato: `GUI-U-E3-T09`
- Massimo due successivi: `GUI-U-E3-T10`, `GUI-U-E3-T11`
...
- Blocchi aperti: `GUI-U-E3-T09 = BLOCKED`; OAuth esclusivo e` approvato per Gmail/Drive, ma manca il client OAuth Desktop centrale di Caronte da fornire come input locale ignorato della build; `GUI-U-E3-T10` richiede ancora decisione sulla responsabilita` di selezione del Registro Google
...
- Ultima suite locale: `local_connector\.venv\Scripts\python.exe -m pytest -o addopts='' --basetemp $env:TEMP\virgilio-pytest-gui-e3-t08-full local_connector -q` -> `OK` (`454 passed`)
- Ultimo smoke locale: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\dev\smoke_local_connector.ps1` -> `OK` (`454 passed`)
