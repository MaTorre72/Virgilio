"""Managed background execution for the local Caronte GUI."""
...
class ManagedCliRunner:
    """Own one CLI child process and expose its lifecycle without blocking Tk."""

    def __init__(self, *, process_factory: Callable[..., object] = subprocess.Popen,
    ...
    def start(self, args: list[str]) -> bool:
        with self._lock:
            if self._state in {"starting", "running", "stopping"}:
                self._events.put(RunnerEvent("rejected", self._state,
                                             "Caronte e` gia` in esecuzione."))
                return False
