...
from .local_config_yaml import LocalConfigYamlError, parse_local_config_yaml
...
def _parse_config_yaml(path: Path) -> tuple[list[dict[str, object]], dict[str, object] | None]:
    try:
        return parse_local_config_yaml(path)
    except LocalConfigYamlError as exc:
        raise MultiAccountConfigError(str(exc)) from exc
